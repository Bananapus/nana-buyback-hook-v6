...
| `JBPayHookSpecification` | `hook` (IJBPayHook), `amount` (uint256), `metadata` (bytes) | Returned from `beforePayRecordedWith` when swap is chosen. `metadata` encodes `(bool projectTokenIs0, uint256 mintFromExcess, uint256 minimumSwapAmountOut, IJBController controller, uint256 tokenCountWithoutHook)`. The 5th field is informational for preview clients; `afterPayRecordedWith` decodes only the first 4. |
...
- **Hook spec metadata**: When the swap path is chosen, `JBPayHookSpecification.metadata` encodes 5 fields: `(bool projectTokenIs0, uint256 mintFromExcess, uint256 minimumSwapAmountOut, IJBController controller, uint256 tokenCountWithoutHook)`. Only the first 4 are decoded by `afterPayRecordedWith`. The 5th (`tokenCountWithoutHook`) is informational for preview clients (e.g., `JBTerminalStore.previewPayFrom`).
