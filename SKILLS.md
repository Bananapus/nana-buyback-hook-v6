...
| `JBPayHookSpecification` | `hook` (IJBPayHook), `amount` (uint256), `metadata` (bytes) | Returned from `beforePayRecordedWith` when swap is chosen. `metadata` encodes 8 fields: `(bool projectTokenIs0, uint256 mintFromExcess, uint256 minimumSwapAmountOut, IJBController controller, uint256 tokenCountWithoutHook, int24 twapTick, uint128 twapLiquidity, PoolId poolId)`. `afterPayRecordedWith` only decodes the first 4; fields 5-8 are informational for preview clients. |
