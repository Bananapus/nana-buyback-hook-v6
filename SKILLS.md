...
| `JBPayHookSpecification` | `hook` (IJBPayHook), `noop` (bool), `amount` (uint256), `metadata` (bytes) | Returned from `beforePayRecordedWith` for both active swap specs and noop informational mint-path specs. `metadata` encodes 10 fields: `(bool projectTokenIs0, uint256 mintFromExcess, uint256 minimumSwapAmountOut, IJBController controller, uint256 tokenCountWithoutHook, int24 twapTick, uint128 twapLiquidity, PoolId poolId, uint256 estimatedBeneficiaryTokenCount, uint256 estimatedReservedTokenCount)`. `afterPayRecordedWith` only decodes the first 4; fields 5-10 are informational for preview clients. |
