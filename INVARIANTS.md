...
- **A.1.2 Combined-output floor: explicit hard floor, derived unwind.** When the pool path is chosen, explicit caller minima (`hasUserSpecifiedQuote`) are hard floors on `exactSwapAmountOut + partialMintTokenCount`, even when the swap reverts. Oracle-derived minima are routing hints enforced inside the unlock callback, pro-rated by consumed input: a fill below the pro-rated floor reverts the unlock, `_swap`'s try/catch unwinds it, and the full payment degrades to mint-only fallback — the payment itself never reverts on a derived floor (`JBBuybackHook_DerivedFloorNotMet` is always caught).
- **A.1.3 Reserved-rate preservation.** Tokens received from the pool swap are burned and re-minted through the controller with `useReservedPercent: true`, so the reserved-rate split applies identically whether the route was pool or mint. The only exception is a payer who set the `skipSplits` word in their `pay` metadata: their swap output is transferred to the beneficiary as-is (nothing is issued, so nothing is reserved), and the pool then only has to beat the beneficiary's share of a direct mint. Payments without that word — every programmatic pay — keep the split (`src/JBBuybackHook.sol:588-614`).
...
- **A.1.5 Slippage revert is explicit-only.** If the swap successfully fills (fully or partially) but the combined output falls short of an EXPLICIT caller minimum, the call reverts `JBBuybackHook_SpecifiedSlippageExceeded` rather than silently delivering less than promised. A derived floor miss instead unwinds the swap to the mint fallback (A.1.2), so no-quote payments cannot be bricked by a stale TWAP floor and a manipulated fill nets the payer no less than the issuance-rate mint.
...
- **A.1.7 Same-terminal split normalization.** Same-terminal split pays can quote the destination payment before terminal fees, then forward the hook only the post-fee net amount. Pay metadata carries the gross quoted swap amount so `afterPayRecordedWith` can scale oracle-derived `minimumSwapAmountOut` and `tokenCountWithoutHook` down to the amount actually forwarded. Caller-supplied minima remain hard settlement guarantees and are not scaled.
...
- **A.4.1 TWAP-based quote, live-liquidity gate, bounded buy-side cold-start fallback.** `_getQuote` checks that the pool is configured and that `PoolManager.getLiquidity(poolId)` is non-zero before using the TWAP oracle. If live liquidity is zero, the function returns `(0, 0, ...)` and the hook degrades to the mint/direct-reclaim path. If TWAP liquidity is zero, cash-outs and diagnostic lookups stay TWAP-only and degrade to the protocol path; no spot-priced sell route exists. Buy-side pays may use the bounded cold-start fallback only when `allowColdStartSpotFallback` is true, the pool uses the configured oracle hook, live liquidity exists, estimated impact is below the cold-start cap, and the quote is discounted by LP fee plus the cold-start haircut (`src/JBBuybackHook.sol:1536-1680`).
...
- **D.4 Cold, uninitialized, or drained pools degrade to the protocol path, except for the bounded buy-side bootstrap path.** A `(projectId, terminalToken)` pair with `_poolIsSet == false` causes `beforeCashOutRecordedWith` to return the protocol cash-out values unchanged (subject to A.2.2) and causes `beforePayRecordedWith` to fall through to the no-pool branch (subject to the A.1.1 mint floor + explicit-minimum revert at `src/JBBuybackHook.sol:1135-1142`). A pool with no live in-range liquidity or max-impact dust liquidity makes `_getQuote` return zero (A.4.1, A.4.3), so the hook degrades to the protocol path. A live buy-side pool with no usable TWAP can route only through A.4.1's cold-start guardrails; cash-outs remain TWAP-only.
...
| A.1.2 (combined-output floor + explicit-vs-oracle minima) | `src/JBBuybackHook.sol:571-583` |
| A.1.3 (reserved-rate preservation) | `src/JBBuybackHook.sol:588-614` |
