...
- **A.2.2 Explicit-minimum honored on noop.** If the user supplies `cashOutMinReclaimed` and the hook chooses to noop (passthrough), the conservative net direct reclaim is checked against the user minimum and reverts `JBBuybackHook_SpecifiedSlippageExceeded` if it falls short — even when no pool is configured (`src/JBBuybackHook.sol:803–822`).
