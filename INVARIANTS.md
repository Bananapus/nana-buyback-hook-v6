...
- **A.2.7 Sell-side fee-on-transfer safety.** The remint count used to compute the sell is measured as `balanceAfterMint - balanceBefore` (`src/JBBuybackHook.sol:302-314`) and the terminal-token delivery to the beneficiary uses a recipient balance-delta check (`src/JBBuybackHook.sol:360-367`).
...
- **`unlockCallback(bytes data) returns (bytes)`** — only `poolManager`; reverts `CallerNotPoolManager` otherwise. Executes the swap with a `sqrtPriceLimit` derived from `JBSwapLib.sqrtPriceLimitFromAmounts` and settles input / takes output. Uses balance-delta accounting on the take side for fee-on-transfer safety (`src/JBBuybackHook.sol:714-788`).
