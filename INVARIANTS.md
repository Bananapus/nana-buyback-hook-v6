...
- **A.1.3 Reserved-rate preservation.** Tokens received from the pool swap are burned and re-minted through the controller with `useReservedPercent: true`, so the reserved-rate split applies identically whether the route was pool or mint. The only exception is a payer who set the optional `skipSplits` word in their `pay` metadata: their swap output is transferred to the beneficiary as-is (nothing is issued, so nothing is reserved), and the pool then only has to beat the beneficiary's share of a direct mint. Payments without that word — every programmatic pay — keep the split (`src/JBBuybackHook.sol:583-600`).
