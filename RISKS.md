...
- **PoolKey hook field is owner-trusted input.** `setPoolFor` intentionally does not validate `poolKey.hooks`. A project owner or delegate with `SET_BUYBACK_POOL` can point the buyback hook at a pool whose V4 hook is malicious, buggy, or simply not the intended oracle/routing hook. That is a self-harming configuration risk rather than a permission bypass, but it means pool registration should be reviewed as carefully as any other privileged routing change.
...
- **hookData format**: The buyback hook passes `hookData: abi.encode(uint256(0))` to the V4 swap. The `0` value delegates slippage protection to `JBUniswapV4Hook`'s own routing floor rather than specifying a fixed minimum output. The router hook accepts any payload whose first 32 bytes decode to `amountOutMin`; empty bytes (`""`) still revert because no minimum prefix is present.
